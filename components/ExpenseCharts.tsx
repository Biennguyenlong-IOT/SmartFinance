                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ 
                              width: `${Math.min(100, (item.value / (categories.find(c => c.name === item.name)?.budget || totalExpense)) * 100)}%`,
                        ...
                        <div className="flex justify-between items-center mt-1">
                          <p className="text-[10px] font-bold text-slate-400">
                            {categories.find(c => c.name === item.name)?.budget 
                              ? `${Math.round((item.value / (categories.find(c => c.name === item.name)?.budget || 1)) * 100)}% ngân sách`
                              : `${Math.round((item.value / totalExpense) * 100)}% tổng chi`
                            }
                          </p>
                          {categories.find(c => c.name === item.name)?.budget && (
                            <p className="text-[10px] font-bold text-slate-400">
                              Còn {formatCurrency(Math.max(0, (categories.find(c => c.name === item.name)?.budget || 0) - item.value))}₫
                            </p>
                          )}
                        </div>
                      </div>
